...
if [[ "$ci_mode" -eq 1 ]]; then
  sudo env DEBIAN_FRONTEND=noninteractive APT_LISTCHANGES_FRONTEND=none NEEDRESTART_MODE=a \
    apt-get install -y build-essential clang curl dosfstools git grub-efi-amd64-bin jq libwebkit2gtk-4.1-dev libssl-dev libayatana-appindicator3-dev librsvg2-dev live-boot mkosi mtools ovmf pkg-config qemu-system-x86 qemu-utils squashfs-tools xorriso zstd
  export CI=true
...
missing=0
for tool in cargo mkosi xorriso qemu-system-x86_64; do command -v "$tool" >/dev/null || { echo "MISSING $tool"; missing=1; }; done

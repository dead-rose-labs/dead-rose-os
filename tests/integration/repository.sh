...
rg -q '^[[:space:]]+fdisk$' "$project_dir/os/installer/mkosi.conf"
...
rg -q -- '--run-application' "$project_dir/crates/session/src/main.rs"
rg -q 'core_dumped' "$project_dir/crates/session/src/main.rs"
rg -q 'deadrose\.graphics=safe' "$project_dir/crates/session/src/main.rs"
rg -q 'Dead Rose OS Installer \(safe graphics\)' "$project_dir/os/installer/grub.cfg"
rg -q '^OnUnitActiveSec=60s$' "$project_dir/os/systemd/dead-rose-graphical-diagnostics.timer"
! rg -q '^Environment=DEAD_ROSE_SOFTWARE_RENDERING=1$' "$project_dir/os/systemd/greetd-installer.conf"
rg -q 'InstallerErrorBoundary' "$project_dir/apps/installer/src/main.tsx"
rg -q 'report_frontend_error' "$project_dir/apps/installer/src-tauri/src/main.rs"
...
rg -q -- '--features dead-rose-shell/custom-protocol' "$project_dir/scripts/build-os.sh"
rg -q -- '--features dead-rose-installer/custom-protocol' "$project_dir/scripts/build-iso.sh"
rg -q 'not\(debug_assertions\).*not\(feature = "custom-protocol"\)' "$project_dir/apps/shell/src-tauri/src/main.rs"
rg -q 'not\(debug_assertions\).*not\(feature = "custom-protocol"\)' "$project_dir/apps/installer/src-tauri/src/main.rs"
rg -q 'DEAD_ROSE_FRONTEND_READY' "$project_dir/apps/shell/src-tauri/src/main.rs"
rg -q 'DEAD_ROSE_FRONTEND_READY' "$project_dir/apps/installer/src-tauri/src/main.rs"
rg -q 'frontend-development-url' "$project_dir/os/diagnostics/graphical-session-check"

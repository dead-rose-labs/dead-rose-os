...
[[ -x "$installer_root/usr/bin/apt-cache" ]] || fail "Curtin apt-cache dependency is missing"
[[ -x "$installer_root/usr/bin/ip" ]] || fail "Curtin network diagnostics dependency is missing"
[[ -x "$installer_root/usr/bin/lsb_release" ]] || fail "Curtin release detection dependency is missing"
[[ -x "$installer_root/usr/bin/sudo" ]] || fail "Curtin error-report sudo dependency is missing"
[[ -x "$installer_root/usr/bin/lshw" ]] || fail "Curtin error-report lshw dependency is missing"
...
grep -Fq 'extract_root_tgz_url' "$installer_root/usr/lib/python3/dist-packages/curtin/commands/extract.py" || fail "Curtin rootfs extraction support is missing"
[[ -s "$installer_root/usr/lib/dead-rose-installer/dead-rose-os.rootfs.tar.gz" ]] || fail "Ubuntu rootfs payload is missing"
[[ -s "$installer_root/usr/lib/dead-rose-installer/dead-rose-os.rootfs.tar.gz.sha256" ]] || fail "rootfs payload checksum is missing"
...
[[ "$(readlink "$installer_root/etc/resolv.conf")" == "/run/systemd/resolve/stub-resolv.conf" ]] || fail "installer resolver link is invalid"
grep -Fq 'Environment=DEAD_ROSE_PAYLOAD=/usr/lib/dead-rose-installer/dead-rose-os.rootfs.tar.gz' "$installer_root/usr/lib/systemd/system/dead-rose-installer-backend.service" || fail "installer backend payload path does not name the rootfs archive"

...
grep -Fq "'dd-bmap': ''" "$installer_root/usr/lib/python3/dist-packages/curtin/commands/block_meta.py" || fail "Curtin block-map image support is missing"
[[ -x "$installer_root/usr/bin/bmaptool" ]] || fail "bmaptool is missing from installer root"
[[ -s "$installer_root/usr/lib/dead-rose-installer/dead-rose-os.raw" ]] || fail "sparse installer payload is missing"
[[ -s "$installer_root/usr/lib/dead-rose-installer/dead-rose-os.raw.bmap" ]] || fail "installer payload block map is missing"
[[ -s "$installer_root/usr/lib/dead-rose-installer/dead-rose-os.raw.bmap.sha256" ]] || fail "installer block-map checksum is missing"
...
grep -Fq 'Environment=DEAD_ROSE_PAYLOAD=/usr/lib/dead-rose-installer/dead-rose-os.raw' "$installer_root/usr/lib/systemd/system/dead-rose-installer-backend.service" || fail "installer backend payload path does not name the sparse image"

...
systemd + NetworkManager
  ...
  └─ greetd → deadrose user → Cage → Dead Rose Tauri Shell
```

The `deadrose` graphical account has no password, login shell, or sudo. Core is the only privileged Dead Rose application component. The protocol is newline-delimited typed JSON with a 64 KiB frame limit; no generic command API exists.
